...
* [Средняя длительность сессии](#средняя-длительность-сессии) в общем и по пользователю. Если у какого-то пользователя низкая длительность сессии относительно средней, то, возможно, ему нужна помощь.
* [Какие задачи самые популярные](#какие-задачи-самые-популярные) - можно предлагать эти задачи, чтобы заинтересовать студентов
* [Процент успешных попыток по задаче](#процент-успешных-попыток-по-задаче) - позволяет оценить сложность задачи 
* [Сколько у студента ушло попыток на решение задачи](#сколько-у-студента-ушло-попыток-на-решение-задачи) - тоже может помочь оценить на сложность задачи.
...
* [MAU (month active users) и WAU](#mau-month-active-users-и-wau) - может быть корпоративные пользователи почти не заходят на платформу, а если и заходят, то ничего не делают
...
* [Средняя длительность сессии](#средняя-длительность-сессии-1)
* [Количество заходов без активности](#количество-заходов-без-активности)
* [Удержание и отток](#удержание-и-отток) (Т.к. платформа образовательная, не обязательно, чтобы пользователи заходили каждый день, поэтому буду считать rolling retention и rolling chorn rate, чтобы оценить общую вовлеченность пользователей)
...
* [Сколько задач было решено всего или с разбивкой по сложности или по языкам](#сколько-задач-было-решено-всего-или-с-разбивкой-по-сложности-или-по-языкам)


## Маркетолог
Маркетолог хочет знать информацию, с помощью которой он может прорекламировать продукт компании другим пользователям
* [Сколько всего задач с разбивкой по языкам и сложности](#сколько-всего-задач-с-разбивкой-по-языкам-и-сложности) - можно завлечь большим количеством задач
* [Сколько домашних задач](#сколько-домашних-задач)
* [Сколько тестов](#сколько-тестов)
* [Топ 20 популярных задач](#топ-20-популярных-задач-1) - даст возможность прорекламировать задачи, интересные пользователям
* [Количество пользователей на платформе](#количество-пользователей-на-платформе) и [их активность](#их-активность) - чем больше пользователей и чем выше активность, тем выше заинтересованность других пользователей

## Методисты
Методистам важна эффективность обучающей программы. Их интересует:
* [Сколько задач решено, а сколько нет](#сколько-задач-решено-а-сколько-нет)
* [Топ 20 задач, до которых доходят](#топ-20-задач-до-которых-доходят), но решают не все
* [Распределение студентов по задачам](#распределение-студентов-по-задачам)
* [Сколько студентов решают задачи по каждому из языков](#сколько-студентов-решают-задачи-по-каждому-из-языков)
* [Сколько студентов решают задачи разной сложности](#сколько-студентов-решают-задачи-разной-сложности)
* [Сколько студентов приступают к решению задачи и бросают](#сколько-студентов-приступают-к-решению-задачи-и-бросают)
* [Сколько задач было решено с первой попытки](#сколько-задач-было-решено-с-первой-попытки)
* [Сколько тестов решено и сколько нет](#сколько-тестов-решено-и-сколько-нет)
* [На каких тестах и задачах возникает больше всего трудностей](#на-каких-тестах-и-задачах-возникает-больше-всего-трудностей)
* [Детальная информация по каждому тесту (вопросы и ответы)](#детальная-информация-по-каждому-тесту-вопросы-и-ответы)
