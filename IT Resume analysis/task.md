...
* Удержание (retention) и отток (churn rate) - может корпоративные студенты резко "отпадают" спустя какой-то срок (Т.к. платформа образовательная, не обязательно, чтобы пользователи заходили каждый день, поэтому буду считать rolling retention и rolling chorn rate, чтобы оценить общую вовлеченность пользователей)
...
* [Популярные задачи](#популярные-задачи) - можно смотреть на кол-во попыток решений и на количество юзеров, которые решали эту задачу
* [Количество и процент успешно решенных/нерешенных в итоге задач](#количество-и-процент-успешно-решенныхнерешенных-в-итоге-задач)
...
* [Сколько в среднем попыток уходит на конкретную задачу, выделить с самым большим количеством](#сколько-в-среднем-попыток-уходит-на-конкретную-задачу-выделить-с-самым-большим-количеством)
* [У каких задач низкий процент решений](#у-каких-задач-низкий-процент-решений)
...
* [Какие тесты и сколько решали](#какие-тесты-и-сколько-решали)
* [Какие тесты не решали или решали очень мало](#какие-тесты-не-решали-или-решали-очень-мало)
* [Средний процент правильных ответов в тесте](#средний-процент-правильных-ответов-в-тесте)
...
# Генеральный директор
Т.к. много показателей пересекается с финансовым директором, я буду просто оставлять ссылку на фрагмент в тексте.  
